...
def test_stream_scan_job_status_returns_final_event() -> None:
    ...
    with client.stream(
        "GET", f"/assessments/{assessment_id}/scan-jobs/{scan_job_id}/stream", headers=headers
    ) as response:
...
def test_stream_scan_job_status_returns_404_for_unknown_job() -> None:
    ...
    response = client.get(
        f"/assessments/{assessment_id}/scan-jobs/{unknown_scan_job_id}/stream", headers=headers
    )
